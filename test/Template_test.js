...
describe('Template wrapper', () => {
  ...
  beforeEach(async () => {
    ...
    template = await makeTemplateDirectCall(web3Wrapper, owner, offeringType, issuerJurisdiction, accredited, KYC, details, expires, fee, quorum, vestingPeriod);
  ...
  it('addJurisdiction, checkIfJurisdictionIsAllowed', async () => {
    ...
    await template.addJurisdiction(accounts[0], [canada, usa, india], [true, true, true]);

    const canadaShouldBeUpperCase = "CAN-ON";

    const afterCanada = await template.checkIfJurisdictionIsAllowed(canadaShouldBeUpperCase);
  ...
  it('addRoles, checkIfRoleIsAllowed', async () => {
    ...
    assert.equal(beforeInvestor, false, "Investor status was not read properly");
    assert.equal(beforeDelegate, false, "Delegate status was not read properly");
  ...
  it('updateTemplateDetails, getTemplateDetails', async () => {
    ...
    assert.equal(emptyStringError, "Details cannot be an empty string", 'Custom error message did not work')
  })
  ...
  it('finalizeTemplate, getTemplateDetails', async () => {
    ...
    assert.equal(templateShouldBeFalse, false, 'Template has already been finalized')
  ...
  it('checkTemplateRequirements should return true when the requirements given are all met', async () => {
    ...
    const areRequirementsMet = await template.checkTemplateRequirements(jurisdiction, accredited, role);

    assert.equal(areRequirementsMet, true, 'Passing template requirements threw a failure from polymath.js')
  })
  ...
  it('checkTemplateRequirements should return false when a requirement is not met', async () => {
    ...
    await template.addJurisdiction(accounts[0], [jurisdiction], [true]);
    await template.addRoles(accounts[0], [role]);
    const areRequirementsMet = await template.checkTemplateRequirements(jurisdiction, accredited, roleNotAdded);

    assert.equal(areRequirementsMet, false, 'Failing template requirements passed from polymath.js')
  })
  ...
  it('getTemplateUsageDetails', async () => {
    ...
    assert.equal(templateUsageDetails[0], beforeEachFee, "Fee was not properly received");
    assert.equal(templateUsageDetails[1], beforeEachQuorum, "Quorum was not properly received");
    assert.equal(templateUsageDetails[2], beforeEachVestingPeriod, "Vesting period was not properly received");
    assert.equal(templateUsageDetails[3], beforeEachOwner, "Owner address was not properly received");
    assert.equal(templateUsageDetails[4], beforeEachKYC, "KYC address was not properly received");
  })

  it('getOfferingType', async () => {
    const beforeEachOffering = "Public sale";
    const offeringType = await template.getOfferingType();
    assert.equal(offeringType, beforeEachOffering, "Offering type was incorrectly read");
  })
  ...
  it('getIssuerJurisdiction', async () => {
    ...
    assert.equal(issuerJurisdiction, beforeEachJurisdiction, "Issuer jurisdiction was incorrectly read");
  })

  it('checkIfAccreditationIsRequired', async () => {
    const accreditation = await template.checkIfAccreditationIsRequired();
    assert.equal(accreditation, false, "Accreditation was not correctly checked and compared to false");
  })

  it('getTemplateExpiry', async () => {
    const expiry = await template.getTemplateExpiry();
    const expireTime = 1602288000;
    assert.equal(expiry, expireTime, "Expiry was not correctly checked and compared");
  })
